function BER1m = egc1m(M, frLen, numPackets, EbNo)
%EGC1M  Equal Gain Combining for 1xM antenna configurations.
%
%   BER1M = EGC1M(M, FRLEN, NUMPACKETS, EBNOVEC) computes the bit-error rate 
%   estimates via simulation for a Equal Gain Combined configuration using 
...
%       M = 1 to 4; FRLEN = 100; NUMPACKETS = 1000; EBNOVEC = 2:2:20;
%
%   Example:
%       ber12 = egc1m(2, 100, 1000, 2:2:20);
%
%   See also MRC1M, OSTBC2M, OSTBC4M.
...
%   Derived from the function mrc1m (The MathWorks, Inc.); Version 2.7.2024
...
for idx = 1:length(EbNo)
    ...
    for packetIdx = 1:numPackets
        data = randi([0 P-1], frLen, 1);   % data vector per user
                                                  % per channel;
        tx = pskmod(data, P);  
        ...
        for i = 1:M
            z(:, i) = r(:, i).*exp(-1i*angle(H(:, i)));
        end

        % ML Detector (minimum Euclidean distance)
         demod1m = pskdemod(sum(z, 2), P); % MR combined 
